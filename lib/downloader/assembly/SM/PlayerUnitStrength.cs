...
{
  [Serializable]
  public class PlayerUnitStrength : KeyCompare
  {
    [SerializeField]
    ...
    public PlayerUnitStrength()
    {
    }

    public PlayerUnitStrength(Dictionary<string, object> json)
    {
      this._hasKey = false;
    ...
    public int buildupMaxCnt(PlayerUnit unit)
    {
      ...
      if (num > unit.unit.buildup_limit_release_id.strength_limit_release_cnt)
        num = unit.unit.buildup_limit_release_id.strength_limit_release_cnt;
    ...
    public int possibleBuildupCnt(PlayerUnit unit)
    {
      ...
      if (num > unit.unit.buildup_limit_release_id.strength_limit_release_cnt)
        num = unit.unit.buildup_limit_release_id.strength_limit_release_cnt;
      if (num > unit.unit.buildup_limit_release_id.strength_limit_release_cnt - this.buildup)
        num = unit.unit.buildup_limit_release_id.strength_limit_release_cnt - this.buildup;
      if (num > this.level_up_max_status - this.level)
        num = this.level_up_max_status - this.level;
      return num;
    }
