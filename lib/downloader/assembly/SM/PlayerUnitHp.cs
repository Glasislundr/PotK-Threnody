...
namespace SM
{
  ...
  public class PlayerUnitHp : KeyCompare
  {
    ...
    public int buildupMaxCnt(PlayerUnit unit)
    {
      ...
      if (num > unit.unit.buildup_limit_release_id.hp_limit_release_cnt)
        num = unit.unit.buildup_limit_release_id.hp_limit_release_cnt;
    ...
    public int possibleBuildupCnt(PlayerUnit unit)
    {
      ...
      if (num > unit.unit.buildup_limit_release_id.hp_limit_release_cnt)
        num = unit.unit.buildup_limit_release_id.hp_limit_release_cnt;
      if (num > unit.unit.buildup_limit_release_id.hp_limit_release_cnt - this.buildup)
        num = unit.unit.buildup_limit_release_id.hp_limit_release_cnt - this.buildup;
      if (num > this.level_up_max_status - this.level)
        num = this.level_up_max_status - this.level;
      return Mathf.Max(num, 0);
    }
