  ...
  public class AmazonAppStore : FlowWithPurchaseKit
  {
    ...
    protected override IWebTask CreateFulfillmentTask(PurchaseKit.PurchaseResponse response)
    {
      ...
      foreach (PurchaseKit.PurchaseData purchaseData in ((PurchaseKit.AbstractResponse<PurchaseKit.MetaPurchase, PurchaseKit.PurchaseData>) response).Values)
      {
        ...
        string data0 = ((PurchaseKit.AbstractResponse<PurchaseKit.MetaPurchase, PurchaseKit.PurchaseData>) response).Meta.Data0;
        ...
        ((WebTask<Verify, Verify.Response>) (fulfillmentTask = (IWebTask) new Verify(data0, currencyCode, (float) price, id).GetTask(WebTaskAttribute.Reliable | WebTaskAttribute.Silent | WebTaskAttribute.Parallel))).OnResponse((VoidCallbackWithError<Verify.Response>) ((r, e) =>
        {
          if (e != null)
            hasError = true;
