...
namespace Gsc.Network
{
  public class UnityErrorLogSender : IEnumerator
  {
    ...
    public static void Initialize() => UnityErrorLogSender._instance = (UnityErrorLogSender) null;
    ...
    public void Send(string message, string stacktrace, LogType logType)
    {
      ...
      if (this.isRunning || SDK.Configuration.Env == null || SDK.Configuration.Env.ClientErrorApi == null)
        return;
    ...
    {
      List<object> stackframes = new List<object>();
      while (stacktrace.MoveNext())
      {
        ...
        if (!string.IsNullOrEmpty(input))
        {
          ...
          if (match.Success)
          {
            ...
            str3 = input;
          stackframes.Add((object) new Dictionary<string, object>()
          {
            {
              "function",
              (object) str3
    ...
    private static Dictionary<string, object> CreateUserData()
    {
      try
      {
        return new Dictionary<string, object>()
        {
          {
            "device_id",
            (object) Session.DefaultSession.DeviceID
          },
          {
            "osname",
            (object) "windows"
          ...
          {
            "platform",
            (object) Gsc.Auth.Device.Platform
          }
        };
      }
      catch (Exception ex)
    ...
    private static Dictionary<string, object> CreateExtraData()
    {
      try
      {
        return new Dictionary<string, object>()
        {
          {
            "available_memory",
            (object) Sys.GetAvailableMemoryBytes()
          },
          {
            "language",
            (object) Sys.GetLanguageLocale()
          },
          {
            "os_info",
    ...
    private void InternalSend(UnityErrorLogSender.ErrorData data)
    {
      ...
      request.CustomHeaders.AddCustomHeaders(GsccBridge.defaultHeaders);
      if (!string.IsNullOrEmpty(Revision.DLCVersion))
        request.CustomHeaders.SetCustomHeader("X-Device-DLCVersion", Revision.DLCVersion);
    ...
    private class Request : Gsc.Network.Request<UnityErrorLogSender.Request, UnityErrorLogSender.Response>
    {
      ...
      public Request(Dictionary<string, object> data) => this.data = data;

      public override string GetPath() => SDK.Configuration.Env.ClientErrorApi;

      public override string GetMethod() => "POST";

      protected override Dictionary<string, object> GetParameters() => this.data;
    }
